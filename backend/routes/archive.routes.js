...
module.exports = function(pool, auth) {
    ...
    try {
      ...
      var sql = `
        ...
        LEFT JOIN (
          ...
          FROM tech_operations_archive WHERE serial_number IS NOT NULL
  ...
  router.post('/', auth, async function(req, res) {
    try {
      ...
      var unlinked = await pool.query('UPDATE products_archive SET assembly_id=NULL, serial_order=NULL WHERE assembly_id=$1', [aid]);
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (it.classifier) {
          ...
          if (laborResult.rows[0].total > 0) laborHours = parseFloat(laborResult.rows[0].total);
        }
      ...
      res.json({ ok: true, added: inserted, updated, serialUpdated });
    } catch(e) { res.status(500).json({ ok: false, error: e.message }); }
  });
