module.exports = function(pool, auth) {
  ...
  // Load ops from archive for assembly
  ...
  router.get('/form-data', auth, async function(req, res) {
    try {
      ...
      var orders = await pool.query("SELECT DISTINCT order_number FROM production_orders WHERE order_number IS NOT NULL AND order_number != '' ORDER BY order_number");
      var customers = await pool.query("SELECT DISTINCT customer FROM production_orders WHERE customer IS NOT NULL AND customer != '' ORDER BY customer");
  ...
  router.post('/fact', auth, async function(req, res) {
    try {
      ...
      var summary = await pool.query(
        "SELECT COUNT(*) AS total_ops, COUNT(*) FILTER (WHERE COALESCE(ready,0) >= COALESCE(quantity,0) AND COALESCE(quantity,0) > 0) AS done_ops FROM production_orders WHERE narad_number=$1",
        [b.naradNumber]
      );
      ...
      console.log('Факт:', b.naradNumber, 'оп.', b.operationNumber, '|', '+годных:'+deltaReady, '→', newReady, '/', qty, '| +брак:', deltaDefects, '→', upd.rows[0].defects);
