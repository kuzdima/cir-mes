  ...
  router.delete('/', auth, async function(req, res) {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ ok: false, error: 'Удаление доступно только администратору' });
      }
      var { classifierCode, productName } = req.body;
      if (!classifierCode || !productName) {
        return res.status(400).json({ ok: false, error: 'Не указан classifierCode или productName' });
      }
