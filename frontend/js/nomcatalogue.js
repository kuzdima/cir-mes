  ...
  tbody.innerHTML = rows
    .map(function (r) {
      ...
      return (
        ...
        aid +
        "</td>" +
        '<td style="font-size:10px;color:var(--text3);font-family:monospace">' +
        sid +
        "</td>" +
        '<td style="' +
        indent +
...
function CatFilterArchive(q) {
  let selectedElement = parseInt(
    document.querySelector(".parentLi span.selected")?.parentElement.dataset.id,
  );
...
function catLoadFromDBPrompt() {
  var assemblyId = prompt(
    "Введите обозначение головного изделия (Assembly_ID):",
  );
...
function findCildren(obj, db = []) {
  ...
  return db.filter((value) => {
    return (
      value.assembly_id === assembly_id &&
      value.parent_id === struct_id &&
      parent_id !== value.struct_id
    );
  });
}
  ...
  navlist.addEventListener("click", (e) => {
    ...
    if (target.classList.contains("hasChildren")) {
      ...
      target.childElementCount === 1
        ? target.replaceChildren(target.children[0], childrenElement)
        : target.replaceChildren(target.children[0]);
