...
function loadTechOps() {
  var q  = document.getElementById('ops-q') ? document.getElementById('ops-q').value : '';
  ...
  api('GET', url).then(function(res) {
    ...
    function badge(v, cls, label) { return v ? '<span class="badge ' + cls + '" style="font-size:9px">' + label + '</span>' : ''; }
    ...
    tb.innerHTML = res.rows.map(function(r) {
      ...
      var safeCode = c(r.classifier_code).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');
      var safeName = c(r.product_name).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/'/g,'&#39;');
      return '<tr data-code="' + safeCode + '" data-name="' + safeName + '" onclick="selectOpsRow(this.dataset.code, this.dataset.name, this)" style="cursor:pointer">'
        // ── ДАННЫЕ ИЗДЕЛИЯ (15 колонок) ──
        + '<td style="' + tb2 + 'font-family:monospace;font-size:10px;color:#60a5fa;white-space:nowrap">' + c(r.classifier_code) + '</td>'
        + '<td style="' + tb2 + bold + '">' + c(r.product_name) + '</td>'
        + '<td style="' + tb2 + 'font-size:10px;color:var(--text3)">' + c(r.notice_number) + '</td>'
        + '<td style="' + tb2 + '">' + (c(r.object_type) ? '<span class="badge b-gray" style="font-size:9px">' + c(r.object_type) + '</span>' : '') + '</td>'
        + '<td style="' + tb2 + 'font-size:10px">' + c(r.material_grade) + '</td>'
        + '<td style="' + tb2 + 'font-size:10px">' + c(r.assortment) + '</td>'
        + '<td style="' + tb2 + 'font-family:monospace;text-align:right">' + c(r.material_amount) + '</td>'
        + '<td style="' + tb2 + 'color:var(--text3)">' + c(r.unit_of_measure) + '</td>'
        + '<td style="' + tb2 + 'font-size:10px">' + c(r.coating) + '</td>'
        + '<td style="' + tb2 + 'font-size:10px">' + c(r.hardness) + '</td>'
        + '<td style="' + tb2 + 'font-size:10px">' + c(r.full_name) + '</td>'
        + '<td style="' + tb2 + 'font-family:monospace;text-align:right">' + c(r.make_qty) + '</td>'
        + '<td style="' + tb2 + 'font-family:monospace;text-align:right">' + c(r.mass_kg) + '</td>'
        + '<td style="' + tb2 + 'font-size:10px">' + c(r.dimensions) + '</td>'
        + '<td style="' + tb2 + 'font-family:monospace;font-size:10px;border-right:2px solid rgba(59,130,246,0.3)">' + c(r.main_order_id) + '</td>'
        // ── ТЕХНОЛОГИЧЕСКИЕ ОПЕРАЦИИ (14 колонок) ──
        + '<td style="font-family:monospace;font-weight:700;text-align:center;color:var(--amber-text)">' + c(r.operation_number) + '</td>'
        + '<td style="font-weight:500;white-space:nowrap;color:#34d399">' + c(r.operation_name) + '</td>'
        + '<td style="font-size:10px;color:var(--text2)">' + c(r.executor) + '</td>'
        + '<td style="font-size:10px;color:var(--text3);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + c(r.machine_resource) + '</td>'
        + '<td style="font-size:10px;color:var(--text3)">' + c(r.operation_comment) + '</td>'
        + '<td style="font-size:10px;color:var(--text3)">' + c(r.tool) + '</td>'
        + '<td style="font-family:monospace;text-align:right">' + c(r.norm_time_hours) + '</td>'
        + '<td style="font-family:monospace;text-align:right">' + c(r.batch_prep_hours) + '</td>'
        + '<td style="text-align:center">' + badge(r.is_outsourcing, 'b-amber', 'Да') + '</td>'
        + '<td style="text-align:center">' + badge(r.is_final, 'b-green', '✓') + '</td>'
        + '<td style="text-align:center">' + badge(r.is_cnc, 'b-blue', 'ЧПУ') + '</td>'
...
function handleTFAction() {
  ...
  document.getElementById('tf-action-hint').style.display = v === 'Найти Старое' ? 'block' : 'none';
}
...
function trySearchExisting() {
  ...
  searchTimeout = setTimeout(function() {
    ...
    api('GET', '/api/tech-ops/search?product_name=' + encodeURIComponent(orderId) + '&classifier=' + encodeURIComponent(drawing)).then(function(res) {
      if (res.ok) {
        fillTFForm(res.commonData, res.operations);
        showToast('Загружено из архива: ' + res.operations.length + ' операций');
      }
    });
  }, 800);
}
...
function guessExecutor(name) {
  ...
  for (var i = 0; i < map.length; i++) {
    var kws = map[i][0]; var ex = map[i][1];
    for (var j = 0; j < kws.length; j++) { if (n.indexOf(kws[j]) !== -1) return ex; }
  }
  return '';
}
...
function tfAddOp() {
  ...
  var optsOps = '<option value="">— Выберите —</option>' + REFS.operations.map(function(o){ return '<option value="' + o + '">' + o + '</option>'; }).join('');
  var optsMach = '<option value="">— Выберите —</option>' + REFS.machines.map(function(m){ return '<option value="' + m + '">' + m + '</option>'; }).join('');

  tr.innerHTML =
    '<td><input type="text" name="opNum" value="' + num + '" placeholder="005"></td>' +
    '<td><select name="opName" onchange="opNameChanged(this)">' + optsOps + '</select></td>' +
    '<td><input type="text" name="opExec" placeholder="Автоматически"></td>' +
    '<td><select name="opMachine">' + optsMach + '</select></td>' +
...
function removeOp(btn) {
  var tbody = document.getElementById('tf-ops-body');
  if (tbody.children.length <= 1) { showToast('Должна быть хотя бы одна операция'); return; }
...
function tfAddAnalog() {
  ...
  div.innerHTML = '<input class="form-inp" name="analog" placeholder="АРВЦ.XXXXXX.XXX" style="flex:1;">'
...
function tfSubmit() {
  ...
  var hasFinal = Array.from(opRows).some(function(r){ return r.querySelector('[name="opFinal"]').checked; });
  if (!hasFinal) errors.push('Хотя бы одна операция должна быть финальной');

  if (errors.length) {
    errEl.innerHTML = '<strong>Ошибки:</strong><ul>' + errors.map(function(e){ return '<li>' + e + '</li>'; }).join('') + '</ul>';
  ...
  var operations = Array.from(opRows).map(function(row) {
    return {
      ...
      normTimeHours:    parseFloat(row.querySelector('[name="opNorm"]').value)  || 0,
      batchPrepHours:   parseFloat(row.querySelector('[name="opBatch"]').value) || 0,
  ...
  var analogDrawings = Array.from(document.querySelectorAll('#tf-analogs-container [name="analog"]'))
    .map(function(i){ return i.value.trim(); }).filter(Boolean);

  var payload = {
    action:         document.getElementById('tf-action').value === 'Найти Старое' ? 'replace' : 'new',
  ...
  api('POST', '/api/tech-ops', payload).then(function(res) {
    if (res.ok) {
      ...
      setTimeout(function() { if (confirm('Данные добавлены. Очистить форму?')) tfClearForm(); }, 1000);
    } else {
...
function tfClearForm() {
  ['tf-orderId','tf-drawing','tf-objectType','tf-mainOrderId','tf-makeQty','tf-comments',
   'tf-noticeNumber','tf-fullName','tf-materialGrade','tf-assortment','tf-materialAmount',
   'tf-unitOfMeasure','tf-coating','tf-hardness','tf-mass','tf-dimensions'].forEach(function(id) {
...
function selectOpsRow(code, name, rowEl) {
  ...
  document.getElementById('ops-selected-info').textContent = 'Выбрано: ' + code + ' · ' + name;
}
...
function deleteSelected() {
  ...
  if (!confirm('Удалить ВСЕ операции для:\n\n' + selectedOps.code + ' · ' + selectedOps.name + '\n\nЭто действие нельзя отменить!')) return;
...
function editSelected() {
  ...
  api('GET', '/api/tech-ops/search?product_name=' + encodeURIComponent(selectedOps.name) + '&classifier=' + encodeURIComponent(selectedOps.code)).then(function(result) {
    if (result.ok && result.commonData) {
      ...
      if (result.operations && result.operations.length) {
        ...
        showToast('Загружено для редактирования: ' + result.operations.length + ' операций');
      }
    }
  });
}