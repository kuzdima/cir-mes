...
function arcLoadFromDB() {
  if (!TOKEN) { setTimeout(arcLoadFromDB, 500); return; }
...
function arcAddChildRow() {
  var tbody = document.getElementById('arc-tbody');
  var rows  = Array.from(tbody.children);
  if (arcSelectedIdx === null) { showToast('Кликните по строке-родителю'); return; }
  ...
  var children = rows.filter(function(r) {
    var rid = r.querySelector('[name="arc-id"]').value.trim();
    return rid.startsWith(parentId + '.') && rid.split('.').length === parentId.split('.').length + 1;
  });
...
function arcBuildRow(id, name, classifier, qty, type) {
  ...
  var opts = ['—','Головное изделие','Деталь','ПКИ','Сборочная единица','Комплект','Сборочная единица/ПКИ']
    .map(function(o) {
      return '<option value="' + (o === '—' ? '' : o) + '"' + (o === type ? ' selected' : '') + '>' + o + '</option>';
    }).join('');

  tr.innerHTML =
    '<td><input type="text" name="arc-id" value="' + id + '" onchange="arcRecalcStructure()" placeholder="ID" style="width:60px"></td>' +
    '<td><div class="combo-wrap"><input type="text" name="arc-name" value="' + (name || '') + '" placeholder="Название изделия" autocomplete="off" onchange="arcUpdateAssemblyId()" oninput="arcAcSearch(this,\'' + rid + '-dd-name\',\'product\')" onfocus="arcAcSearch(this,\'' + rid + '-dd-name\',\'product\')"><div class="combo-dd" id="' + rid + '-dd-name"></div></div></td>' +
    '<td><div class="combo-wrap"><input type="text" name="arc-classifier" value="' + (classifier || '') + '" placeholder="АРВЦ..." autocomplete="off" onchange="arcUpdateAssemblyId()" oninput="arcAcSearch(this,\'' + rid + '-dd-code\',\'classifier\')" onfocus="arcAcSearch(this,\'' + rid + '-dd-code\',\'classifier\')"><div class="combo-dd" id="' + rid + '-dd-code"></div></div></td>' +
    '<td><input type="number" name="arc-qty" value="' + qty + '" min="1" style="width:55px"></td>' +
    '<td><select name="arc-type">' + opts + '</select></td>' +
    ...
    '<td><button class="arc-del" onclick="arcRemoveRow(this)"' + (id === '0' ? ' disabled' : '') + '>×</button></td>';
  
  tr.addEventListener('click', function(e) {
    if (e.target.tagName === 'BUTTON') return;
    Array.from(document.getElementById('arc-tbody').children).forEach(function(r) { 
      r.style.outline = ''; 
    });
    tr.style.outline = '2px solid var(--accent)';
    tr.style.outlineOffset = '-1px';
    arcSelectedIdx = Array.from(document.getElementById('arc-tbody').children).indexOf(tr);
  });
...
function arcRemoveRow(btn) {
  ...
  if (row === tbody.firstElementChild) { showToast('Нельзя удалить корневой элемент'); return; }
  ...
  if (children.length && !confirm('У элемента ' + rowId + ' есть ' + children.length + ' дочерних. Удалить все?')) return;
...
function arcRecalcStructure() {
  Array.from(document.getElementById('arc-tbody').children).forEach(function(row) {
    var id = row.querySelector('[name="arc-id"]').value.trim();
    var lvl = id === '0' ? 0 : (id.match(/\./g) || []).length + 1;
    var parentId = id.indexOf('.') !== -1 ? id.split('.').slice(0, -1).join('.') : '0';
...
function arcSubmit() {
  ...
  var data = Array.from(document.getElementById('arc-tbody').children).map(function(row) {
    return {
  ...
  var rootElement = data.find(function(d) { return d.id === '0'; });
...
// Проверяем обязательные поля (ПКИ может быть без обозначения)
  data.forEach(function(d, i) {
    ...
    if (!d.name) errors.push('Строка ' + (i + 1) + ': Название изделия обязательно');
    if (!d.classifier && d.objectType !== 'ПКИ') {
      errors.push('Строка ' + (i + 1) + ': Обозначение обязательно (кроме ПКИ)');
    }
  });
  
  // Проверяем уникальность ID
  var ids = data.map(function(d) { return d.id; });
  var dups = ids.filter(function(id, i) { return ids.indexOf(id) !== i; });
  if (dups.length) errors.push('Дубликаты ID: ' + dups.filter(function(v, i, a) { return a.indexOf(v) === i; }).join(', '));
  
  // Проверяем обозначение корневого элемента
  if (rootElement && !rootElement.classifier) {
    errors.push('У корневого элемента не указано обозначение по классификатору');
  }
  ...
  api('GET', '/api/archive?assembly_id=' + encodeURIComponent(assemblyId)).then(function(res) {
    if (res.ok && res.rows && res.rows.length > 0) {
      ...
      if (warnEl) {
        warnEl.innerHTML = '<strong>Изделие с Assembly_ID "' + assemblyId + '" уже существует в архиве.</strong>'
...
function arcCalcSerialOrder(data) {
  var items = data.map(function(d) {
    ...
    var isAssembly = (d.objectType || '').indexOf('Сборочная') !== -1 || d.objectType === 'Головное изделие';
    return { id: d.id, name: d.name, objectType: d.objectType, level: lvl, isAssembly: isAssembly };
  });
...
function arcDoSubmit(force) {
  ...
  var items = data.map(function(d) {
    ...
    var parentId = d.id.indexOf('.') !== -1 ? d.id.split('.').slice(0, -1).join('.') : '0';
    return {
      id: d.id,
      name: d.name,
  ...
  var manufactured = items.filter(function(i) { return i.serialOrder; }).length;
  var assemblyId = data.find(function(d) { return d.id === '0'; }).classifier;
  
  // Показываем порядок в консоли
  console.log('Порядок изготовления для ' + assemblyId + ':');
  items.filter(function(i) { return i.serialOrder; })
    .sort(function(a, b) { return a.serialOrder - b.serialOrder; })
    .forEach(function(i) {
      console.log('  №' + i.serialOrder + ': ' + i.name + ' (' + (i.classifier || 'без обозначения') + ')');
    });
  
  api('POST', '/api/archive', { items: items, force: force }).then(function(res) {
    if (res.ok) {
      ...
      if (okEl) {
        ...
        okEl.innerHTML = '✓ Головное изделие: ' + assemblyId
          + '<br>Привязано: ' + upd + ' | Новых: ' + ins + ' | Порядок: ' + manufactured + ' поз.'
      ...
      setTimeout(function() {
        if (confirm('Данные добавлены в архив. Очистить форму?')) arcClearForm();
      }, 800);
    } else {
      var errEl = document.getElementById('arc-err');
      if (errEl) {
        errEl.textContent = '✗ Ошибка: ' + (res.error || 'неизвестная ошибка');
        errEl.classList.add('show');
      }
    }
  });
...
function arcClearArchive() {
  if (!confirm('Очистить ВЕСЬ архив изделий? Это действие нельзя отменить!')) return;
...
function arcExportCSV() {
  if (!arcDB.length) { showToast('Архив пуст'); return; }
  
  var headers = ['ID', 'Название изделия', 'Обозначение по классификатору', 'Кол-во в изделии (шт.)', 'Тип объекта', 'Трудоемкость', 'Родительский ID', 'Уровень', 'Assembly_ID'];
...
function arcLoadStructure(assemblyId) {
  api('GET', '/api/archive?assembly_id=' + encodeURIComponent(assemblyId)).then(function(res) {
    ...
    showToast('Загружена структура: ' + assemblyId + ' (' + rows.length + ' элементов)');
  });
}
    ...
    http('GET', '/api/tech-ops/autocomplete?q=' + encodeURIComponent(q) + '&field=' + field, null, function(res) {
      ...
      res.rows.forEach(function(r) {
        ...
        item.addEventListener('click', function() {
          ...
          arcUpdateAssemblyId();
          setTimeout(function() { arcAcLock = false; }, 500);
          ...
          if (idInput && idInput.value.trim() === '0') {
            ...
            var checkAssembly = (field === 'classifier') ? cleanName : cleanCode;
            if (checkAssembly) {
              api('GET', '/api/archive?assembly_id=' + encodeURIComponent(checkAssembly)).then(function(arcRes) {
                if (arcRes.ok && arcRes.rows && arcRes.rows.length > 1) {
                  if (confirm('Найдена существующая структура для "' + checkAssembly + '" (' + arcRes.rows.length + ' элементов).\n\nЗагрузить для редактирования?')) {
