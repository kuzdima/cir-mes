...
function comboHideAll() { document.querySelectorAll('.combo-dd').forEach(function(d){ d.classList.remove('show'); }); }
    ...
    http('GET', '/api/tech-ops/autocomplete?q=' + encodeURIComponent(q) + '&field=' + field, null, function(res) {
      ...
      res.rows.forEach(function(r) {
        ...
        item.addEventListener('click', function() {
          ...
          setTimeout(function() { acLock = false; }, 500);
          ...
          api('GET', '/api/tech-ops/search?product_name=' + encodeURIComponent(productName) + '&classifier=' + encodeURIComponent(classifierCode)).then(function(result) {
            if (result.ok && result.commonData) {
              ...
              if (result.operations && result.operations.length) {
                ...
                showToast('Загружено: ' + result.operations.length + ' операций');
              } else {
